    ...
    def update(self):
        ...
        for state,reward, is_terminal in zip(reversed(self.buffer.states),reversed(self.buffer.rewards), reversed(self.buffer.is_terminals)):
            ...
            if idx==0:
                ...
                rewards_=torch.cat((rewards_, discounted_reward),0)
            idx+=1
        rewards_=rewards_.squeeze()
        ...
        for _ in range(self.K_epochs):
            for i in range(int(nums/batch)):
                ...
                loss =  0.5*self.MseLoss(state_values, rewards.detach())+rewards

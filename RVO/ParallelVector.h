...
template <typename T,typename Op=ReduceAdd<T>>
class ParallelValue {
 public:
  typedef std::vector<T,Eigen::aligned_allocator<T> > vector_type;
  typedef typename std::vector<T,Eigen::aligned_allocator<T> >::const_iterator const_iterator;
  typedef typename std::vector<T,Eigen::aligned_allocator<T> >::iterator iterator;
  ParallelValue() {}
  ParallelValue(T example) {
    assign(omp_get_num_procs(),example);
  }
  ParallelValue(int nr,T example) {
    assign(nr,example);
  }
  ...
  void clear() {
    _blocks[0]=Op::init(_blocks[0]);
    assign((int)_blocks.size(),_blocks[0]);
  }
  template <typename TOTHER>
  ParallelValue<T,Op>& reduce(const TOTHER& other) {
    _blocks[id()]=Op::reduce(_blocks[id()],other);
    return *this;
  }
  ...
  const T& getValue() const {
    const_cast<ParallelValue<T>*>(this)->join();
    return _blocks[0];
  }
  T& getValue() {
    join();
 ...
 protected:
  ...
  void join() {
    for(int i=1; i<(int)_blocks.size(); i++) {
      _blocks[0]=Op::reduce(_blocks[0],_blocks[i]);
      _blocks[i]=Op::init(_blocks[0]);
    }
  }
  std::vector<T> _blocks;
};
template <typename Vec,typename Op=ReduceAdd<Vec>>
class ParallelMatrix {
 public:
  ...
  template <typename TOTHER>
  ParallelMatrix<Vec,Op>& reduce(const TOTHER& other) {
    _blocks[id()]=Op::reduce(_blocks[id()],other);
    _joined=false;
    return *this;
  }
  ...
  const Vec& getMatrix() const {
    const_cast<ParallelMatrix<Vec>*>(this)->join();
    return _blocks[0];
  }
  Vec& getMatrix() {
    join();

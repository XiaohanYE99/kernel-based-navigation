...
PYBIND11_MODULE(pyRVO, m) {
  ...
  .def_static("assembleAgentId",&SourceSink::assembleAgentId)
  .def_static("extractSourceId",&SourceSink::extractSourceId)
  .def_static("extractAgentId",&SourceSink::extractAgentId)
  ...
  .def("getAgentRadius",[](std::shared_ptr<RVOSimulator> sim)->RVOSimulator::Vec {return sim->getAgentRadius();})
  .def("getAgentId",[](std::shared_ptr<RVOSimulator> sim)->RVOSimulator::Veci {return sim->getAgentId();})
  ...
  .def("getAllAgentRadius",&MultiRVOSimulator::getAllAgentRadius)
  .def("getAllAgentIds",&MultiRVOSimulator::getAllAgentIds)

...
class PolicyNet(nn.Module):
    def __init__(self, env, state_dim, action_dim, has_continuous_action_space, action_std_init=0.2
        ...
        if has_continuous_action_space:
            ...
            self.actor = nn.Sequential(
                ...
                nn.Linear(128, 2*self.env.n_robots), nn.Sigmoid()
            )
            '''
    ...
    def implement(self, state, target,training):
        ...
        v = 8*torch.squeeze(self.actor(I), 1)-4#+0.5
        ...
        if training:
            xNew_f = self.MultiCFLayer(self.env, state_f, v)
        else:
            xNew_f = self.CFLayer(self.env, state_f, v)
    ...
    def update(self):
        ...
        for i in range(t):
            ...
            for k in range(1000):
                loss = torch.mean(self.MultiCoverageLayer(self.env,state))
                ...
                #print(100 * state.grad)
                state.data = state.data - 100 * state.grad
                state.grad.data.zero_()
                # print(loss.item())
                loss_sum += loss.item()
            #print(self.actor.state_dict()['0.weight'])
            #print(self.env.aim)
                print(loss.item() / self.num_train_steps)
...
if __name__ == '__main__':
    ...
    sim = pyrvo.RVOSimulator(2,10,1e-4,1,2,200)
    multisim = pyrvo.MultiRVOSimulator(batch_size,2,10,1e-4,1,2,200)
    ...
    #policy.actor = torch.load('model/model_120.pth')
    # init sample
    # policy.eval()
    #policy.init_sample()
    sumloss = 0

...
namespace RVO {
	...
	public:
		...
		size_t addAgentCar(const Vector2& position, float neighborDist, 
			size_t maxNeighbors, float timeHorizon,
			float timeHorizonObst, float radius, float maxSpeed, 
			const Vector2& velocity = Vector2(), float lambda = 0.5f);
		...
		AgentCar* getAgentCar(size_t agentNo) const;

		float getAgentCarTheta(size_t agentNo) const;

		float getAgentCarPhi(size_t agentNo) const;
		...
		const std::vector<Line>& getAgentORCA(size_t agentNo) const;
		...
		const Vector2& getAgentNewVelocity(size_t agentNo) const;
		...
		void setAgentLambda(size_t agentNo, float lambda);
